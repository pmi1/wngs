...
$(document).ready(function () {
    ...
    $('[data-open-popup]').magnificPopup({
        ...
        callbacks: {
            ...
            close: function() {
                $('.js--popup-article-slider').slick('unslick');
            }
        }
    });
    ...
    $('.js--slider').each(function () {
        var el = $(this),
            ...
            minValue = fmin.val() ? parseFloat(fmin.val()) : 0,
            maxValue = fmax.val() ? parseFloat(fmax.val()) : max;
    ...
    function scrollWindow(e) {
        ...
        if ($(document).width() >= 1024) {
            ...
            var header = $('.js--header-main'),
                ...
                floatHeader = $('#float-header-id');
            if (wsy > headerBottom - 50) {
                // show float header
    ...
    // js--site-search-oval --- begin
    if ($('.js--site-search-oval-button').length) {
        $('.js--site-search-oval-button').on('click', function (e) {
            var parent = $(this).parent(),
                link = parent.find('.js--site-search-oval-form-close'),
                divForm = parent.find('.js--site-search-oval-form');
            if (divForm.hasClass('hide-form')) {
                // open form
                e.preventDefault();
                divForm.removeClass('hide-form');

                headerSearchResize(true);

                // back layer
                var back = document.createElement('div');
                back.className = 'js--site-search-back-div';
                ...
                back.style.backgroundColor = 'rgba(255,255,255,.05)';
    ...
    if ($('.js--site-search-oval-form-close').length) {
        $('.js--site-search-oval-form-close').on('click', function (e) {
            var divForm = $(this).parent();
            e.preventDefault();
            // close form
            divForm.addClass('hide-form');
            divForm.removeAttr('style');
            if ($('.js--site-search-back-div').length) {
                $('.js--site-search-back-div').remove();
            }
        });
    }
    // js--site-search-oval --- end

    // new header search form resizer --- begin
    function headerSearchResize(mode) {
        var mode = mode || false;
        var header = $('header.site-main header.top'),
            soForm = $('.site-search-oval-form'),
            headerRightSections = header.find('.right > section'),
            headerWidth = header.outerWidth(true),
            hrsWidth = 0,
            sfWidth = soForm.outerWidth(true);

        // search form is closed
        if (!mode && sfWidth <= 0) return;

        var index = 0;
        headerRightSections.each(function () {
            index++;
            //if (index == 1) return;
            var el = $(this);
            hrsWidth += el.outerWidth(true);
        });
        soForm.css({
            width: Math.round(headerWidth - hrsWidth + 50)
        });
    }
    var headerSearchResizeTimer = null;
    window.addEventListener('resize', function (e) {
        clearTimeout(headerSearchResizeTimer);
        headerSearchResizeTimer = setTimeout(function () {
            headerSearchResize();
        }, 100);
    });
    // new header search form resizer --- end
    // 02.04.2019 --- end


    // 08.05.2019 --- begin

    // click on tabs control
    $('body').on('click', '.js--site-popup-tab-title', function (e) {
        e.preventDefault();
        var link = $(this);
        ...
        if (id && tab.length) {
            // hide all tabs
            tab.parent().find('.js--site-popup-tab-item').removeClass('active');
            link.parent().find('.js--site-popup-tab-title').removeClass('active');
            // show a tab with ID = id
            tab.addClass('active');
            link.addClass('active');
        }
    });

    // click on accordion items
    $('body').on('click', '.js--savi-title', function (e) {
        e.preventDefault();
        var link = $(this),
            parent = link.parent('.js--sav-item');
        // show the current accordion
        parent.toggleClass('active');
    });

    // extra tooltips
    // Just add the attribute:
    // data-extra-tooltip="SOME TEXT"
    // to an element
    if ($('.js--site-extra-tooltip').length) {

        // Enables tooltip plugin for each tooltip icon
        $('.js--site-extra-tooltip').tooltipster({
            theme: ['tooltipster-light', 'site-extra-tooltip'],
            maxWidth: 170,
            side: ['bottom', 'top']
        });
    }

    // popup content window form (white background)
    $('[data-open-popup-form]').magnificPopup({
        type: 'inline',
        preloader: false,
        modal: true,
        mainClass: 'popup-form-container'
    });

    // js--ste-close (remove a site tag element)
    $('body').on('click', '.js--ste-close', function (e) {
        e.preventDefault();
        var link = $(this).parents('.js--site-tag-element');
        link.remove();
    });

    // [data-popup-form-chooser] (popup form window)
    $('[data-popup-form-chooser]').on('submit', function (e) {
        e.preventDefault();
        var form = $(this),
            formFields = form.find(':input:checked'),
            inputName = form.data('input-name'),
            targetElId = form.data('popup-form-chooser'),
            targetEl = $('#' + targetElId); // an element to put chosen items
        //console.log(formFields);
        if (targetEl) {
            // empty the target element
            targetEl.empty();
            // add items to the target element
            $(formFields).each(function (index, item) {
                var value = $(item).data('value');
                var item = $('<div class="site-tag-element js--site-tag-element">' + '<a href="javascript:void(0);" class="site-tag-element-close js--ste-close"></a>' + value + '<input type="hidden" name="' + inputName + '" value="' + $(item).val() + '">' + '</div>');
                targetEl.append(item);
            });

            if ('product-section-chooser-id' == targetElId) {
                $('#catalogChecker').prop('disabled', formFields.length);
            }
        }
        // close popup
        $.magnificPopup.close();
    ...
    // js--ipi (show preview picture)
    $('body').on('change', '.js--ipi input[type=file]', function (e) {
        var src = URL.createObjectURL(e.target.files[0]),
            parent = $(this).parents('.js--ipi'),
            img = parent.find('img'),
            input = e.target;
        $(parent).parents('.images-product-list').find('.note').remove();
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                $('<div class="note">Изображение загружено, не забудьте сохранить</div>').appendTo($(parent).parents('.images-product-list'));
            };
            reader.readAsDataURL(input.files[0]);
        }
    });

    // js--ipi-remove (remove a file input value)
    $('body').on('click', '.js--ipi-remove', function (e) {
        e.preventDefault();
        var parent = $(this).parents('.js--ipi'),
        ...
        if (parent.find(':input[type=file][name^=media]').length) {
            $('<input name="delete_'+input.attr('name')+'" type="hidden" value="1"/>').insertBefore(input);
            $('<div class="note">Изображение удалено, не забудьте сохранить</div>').appendTo($(parent).parents('.images-product-list'));
        }
    });
